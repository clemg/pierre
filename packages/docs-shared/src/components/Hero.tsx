...
import { getProductConfig, type ProductId } from '../config/products';
import { Button } from './ui/button';
import { Tooltip, TooltipContent, TooltipTrigger } from './ui/tooltip';

export interface HeroProps {
  productId: ProductId;
  /** Current package version, supplied by the calling app. */
  version: string;
}

export function Hero({ productId, version }: HeroProps) {
  ...
  return (
    <section className="flex max-w-3xl flex-col gap-3 pt-20 pb-10 md:pb-20 lg:max-w-4xl">
      ...
      <p className="text-muted-foreground mt-2 text-sm">Currently v{version}</p>
    </section>
  );
}
