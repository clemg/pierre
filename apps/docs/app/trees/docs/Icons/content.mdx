...
Use the `icons` option inside `PathStoreFileTreeOptions` to choose one of the
built-in icon sets or inject your own SVG sprite. Try the live demo at
...
Icons can also be updated at any time after construction using
`fileTree.setIcons(icons)`.

<DocsCodeExample {...pathStoreIcons} />
